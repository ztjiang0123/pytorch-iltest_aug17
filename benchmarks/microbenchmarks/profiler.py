...
def _run_inference_iters(model, input_data, device, iters):
    """Run the model for a number of no-grad iterations, syncing on CUDA."""
    with torch.no_grad():
...
def generate_model_profile(model, input_data, profile_file_path):
    ...
    # Warm up
    _run_inference_iters(model, input_data, device, iters=3)
