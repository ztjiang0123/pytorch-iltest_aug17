...
@dataclass
class _RangeSettingContext:
    """Loop-invariant configuration shared across every module during range setting."""
...
def _get_module_input_data_for_range_setting(
    ...
    if data_getter is not None:
        return data_getter.pop(ctx.model, name)
    return get_module_input_data(ctx.model, ctx.data, module, ctx.batch_size)
...
def _set_weight_range_for_module(
    ...
    ctx.model.apply(all_quantizers_off)
    # TODO: Some form of smart subsampling from all this sequential data
    input_data = _get_module_input_data_for_range_setting(
        ctx, name, module, ctx.data_getter
    )
    output_data = get_batched_output(module, input_data, ctx.batch_size)

    if ctx.progressive:
        ctx.model.apply(all_weight_quantizers_on)
        input_data = _get_module_input_data_for_range_setting(
            ctx, name, module, ctx.data_getter_progressive
        )
    ...
    best_scale = find_optimal_scales_with_loss(module, loss_fn, ctx.num_points)
    module.weight_scale.data = best_scale
...
def set_weight_range_activation_loss(
    ...
    ctx = _RangeSettingContext(
        ...
        data_getter_progressive=data_getter_progressive,
    )
    ...
    with torch.no_grad():
        for name, module in model.named_modules():
            if isinstance(module, QuantizedLinear):
                _set_weight_range_for_module(ctx, name, module)
