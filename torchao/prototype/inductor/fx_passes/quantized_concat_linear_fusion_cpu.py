...
def _compute_fused_output_params(qlinear_users, dtype, gm):
    """Compute the fused (output_scale, output_zero_point) for the concat qlinear."""
    if dtype != torch.uint8:
        fused_output_scale = float(
            max(
                _to_scalar(_get_node_arg(user, "output_scale", 7), gm, float)
    ...
    global_max = None
    for user in qlinear_users:
        scale = _to_scalar(_get_node_arg(user, "output_scale", 7), gm, float)
        zp = _to_scalar(_get_node_arg(user, "output_zero_point", 8), gm, int)
...
def _create_concat_qlinear_node(graph, node, node_before_linear, qlinear_kwargs):
    """Create the fused qlinear node, using positional args for the C++ overloads."""
...
def _update_dequant_scale(
    ...
    """Rewrite a downstream dequant node's scale/zero_point to the fused values."""
    if dtype == torch.uint8:
        dequant_node.args = (
            dequant_node.args[0],
            fused_output_scale,
...
def _update_qparam_kwargs(
    qparam_nodes, qparam_kwarg_keys, dtype, fused_output_scale, fused_output_zero_point
):
    """Fuse SDPA-style q/k/v qparam kwargs when all splits share one consumer."""
    if not (
...
def _fuse_qlinear_group(graph, gm, node, qlinear_users, dtype):
    """Fuse a validated group of qlinear users sharing one activation into one qlinear."""
    ...
    with graph.inserting_before(node):
        ...
        new_qlinear_node = _create_concat_qlinear_node(
...
def _concat_linear_quantized_cpu(graph: torch.fx.Graph):
    ...
    for node in list(graph.nodes):
        ...
        _fuse_qlinear_group(graph, gm, node, qlinear_users, dtype)
