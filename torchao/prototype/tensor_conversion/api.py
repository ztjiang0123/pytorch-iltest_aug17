...
def _convert_linear_weight_for_aarch64(module, tensor_type, intx_packing_format):
    """Convert a single ``nn.Linear`` weight to the requested packed tensor type."""
    weight = module.weight
    if tensor_type == "int8_lut_tensor":
        ...
        # nn.Linear weights with a float zero point can only be packed via the
        # int8 LUT tensor path; everything else uses the opaque tensor.
...
def _convert_model_for_aarch64(
    ...
    *,
    ...
    for name, module in model.named_modules():
        ...
        _convert_linear_weight_for_aarch64(module, tensor_type, intx_packing_format)
