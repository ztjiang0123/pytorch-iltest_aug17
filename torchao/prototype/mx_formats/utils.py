...
def _packed_slice_bounds(x: torch.Tensor, start, end):
    """Return the (start, end) slice bounds for the qdata tensor.

    For FP4-packed data (dtype != float8_e4m3fn) each byte holds two elements,
    so element indices are halved; float8 data is stored one element per byte.
    """
...
def _slice_swizzled_dim0(x, start, end, step, M, n_row_blocks, n_col_blocks):
    ...
    start_idx = start_block * blocks_per_row * _SWIZZLE_ELEMENTS_PER_BLOCK
    end_idx = (
        end_block * blocks_per_row * _SWIZZLE_ELEMENTS_PER_BLOCK
        if end_block < n_row_blocks
        else None
    )

    sliced_scale = aten.slice.Tensor(x.scale.flatten(), 0, start_idx, end_idx, 1)
...
def _slice_swizzled_dim1(x, start, end, step, K, scale_cols, n_row_blocks, n_col_blocks):
    ...
    if start_col_block == 0 and end_col_block == n_col_blocks:
        ...
        elements_per_row_block = n_col_blocks * _SWIZZLE_ELEMENTS_PER_BLOCK

        # Build list of slices to extract
        slices_to_extract = []
        for row_block in range(n_row_blocks):
            row_start = row_block * elements_per_row_block
            col_start = row_start + start_col_block * _SWIZZLE_ELEMENTS_PER_BLOCK
            col_end = row_start + end_col_block * _SWIZZLE_ELEMENTS_PER_BLOCK
    ...
    packed_start, packed_end = _packed_slice_bounds(x, start, end)
    sliced_data = aten.slice.Tensor(x.qdata, 1, packed_start, packed_end, step)
    return sliced_data, sliced_scale
...
def _slice_unswizzled(x, dim, start, end, step, M, K):
    ...
    if dim == 0:
        ...
        if start is not None:
            ...
            assert start % 2 == 0, f"Start index {start} must be even for FP4 packing"
        ...
        packed_start, packed_end = _packed_slice_bounds(x, start, end)
        sliced_data = aten.slice.Tensor(x.qdata, dim, packed_start, packed_end, step)
    ...
    return sliced_data, sliced_scale.flatten()


def _reshape_sliced_scale(x, sliced_data, sliced_scale):
    """Reshape the flattened sliced scale to match the sliced qdata shape."""
    ...
    if x.is_swizzled_scales:
        if x.block_size == 16:
            ...
            scale_M, scale_K = hp_data_dims_to_swizzled_scale_dims_mx(sliced_M, sliced_K)
    else:
    ...
    return sliced_scale.view(scale_M, scale_K)


_SWIZZLE_ELEMENTS_PER_BLOCK = 32 * 16  # 512 elements
