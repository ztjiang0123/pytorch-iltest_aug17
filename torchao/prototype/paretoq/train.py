...
def _init_weight_clip_val(weight_param, w_bits):
    """Compute the initial ``weight_clip_val`` scale for a given bitwidth."""
    if w_bits == 1:
...
def _init_weight_clip_vals(model, w_bits):
    """Populate every ``weight_clip_val`` parameter from its paired weight."""
    ...
    for name, param in model.named_parameters():
        ...
        param.data.copy_(_init_weight_clip_val(weight_param, w_bits))
