...
def _get_static_qparams_decomposed(activation_post_process, dtype):
    """Extract the quantize/dequantize ops and qparams dict for the static
    (uint8/int8/int32) quantization branch."""
    ...
    if is_per_channel(activation_post_process.qscheme):  # type: ignore[attr-defined]
        ...
        qparams = {
            ...
            "_axis_": ch_axis,
            "_quant_min_": activation_post_process.quant_min,
            "_quant_max_": activation_post_process.quant_max,
            "_dtype_": to_underlying_dtype(dtype),
        }
    else:
        ...
        qparams = {
            ...
            "_quant_min_": activation_post_process.quant_min,  # type: ignore[attr-defined]
            "_quant_max_": activation_post_process.quant_max,  # type: ignore[attr-defined]
            "_dtype_": to_underlying_dtype(dtype),
        }
...
def _replace_observer_dynamic_decomposed(
    model, graph, node, activation_post_process, dtype
):
    """Replace observer with choose_qparams + q/dq nodes for the dynamic
    (uint8/int8) quantization branch."""
    # 1. extract information for inserting q/dq node from activation_post_process
...
def _replace_observer_fp16_decomposed(graph, node):
    """Replace observer with to_fp16 -> to_fp32 nodes for the float16 branch."""
