...
def _add_dequantize_op_kwargs(dequantize_op, input_node):
...
def _add_quantize_dequantize_node_info(qdq_node, original_node):
...
def _static_qparams_decomposed(activation_post_process, dtype):
    """Compute (quantize_op, dequantize_op, qparams) for the static branch."""
...
def _replace_observer_static_decomposed(ctx, node, activation_post_process, dtype):
    ...
    # replace activation_post_process node with quantize and dequantize
    with graph.inserting_before(node):
        ...
        quantized_node = graph.create_node(
            "call_function", quantize_op, tuple(quantize_op_inputs), {}
        )
        _add_quantize_dequantize_node_info(quantized_node, node)
        ...
        dequantized_node = graph.call_function(
            dequantize_op,
            tuple(dq_inputs),
            _add_dequantize_op_kwargs(dequantize_op, input_node),
        )

        node.replace_all_uses_with(dequantized_node)

        _add_quantize_dequantize_node_info(dequantized_node, node)
        graph.erase_node(node)


def _insert_choose_qparams_decomposed(graph, node, activation_post_process, dtype):
    """Insert choose_qparams op for the dynamic branch and return the qparams dict."""
    ...
    # extract qparams from activation_post_process module
    dtype_ = to_underlying_dtype(dtype)
    ...
    # insert choose_qparams op and update the qparams list
    with graph.inserting_before(node):
        ...
        return {
            ...
            "_quant_min_": qparams["_quant_min_"],
            "_quant_max_": qparams["_quant_max_"],
            "_dtype_": qparams["_dtype_"],
        }


def _replace_observer_dynamic_decomposed(graph, node, activation_post_process, dtype):
    """uint8/int8/fp16 dynamic quantization branch."""
    quantize_op = torch.ops.quantized_decomposed.quantize_per_tensor.tensor
    qparams = _insert_choose_qparams_decomposed(
        graph, node, activation_post_process, dtype
    )

    # replace activation_post_process node to quantize and dequantize node
    with graph.inserting_before(node):
        input_node = node.args[0]
        quantize_op_inputs = [input_node]
        for key, value_or_node in qparams.items():
...
def _replace_observer_fp16_decomposed(graph, node):
    """float16 branch: insert to_fp16 -> to_fp32 node."""
...
def _replace_observer_with_quantize_dequantize_node_decomposed(
    ...
    # extract the information from activation_post_process module for generating
    ...
    if dtype in SUPPORTED_QDTYPES and (not is_dynamic):
        ...
        _replace_observer_fp16_decomposed(graph, node)
...
def _static_quantize_op_spec(
    ...
    if is_per_channel(activation_post_process.qscheme):  # type: ignore[attr-defined]
        ...
        quantize_op: Callable = torch.quantize_per_channel
    else:
        qparams = {
            "_scale_": float(scale),
            "_zero_point_": int(zero_point),
...
def _insert_quantize_dequantize_nodes(
    ...
    """Replace ``node`` with a quantize op followed by a dequantize call.

    ``_scale_``/``_zero_point_`` qparams are registered as buffers on the root
...
def _should_convert_weighted_module(
    ...
    Collects the guard conditions (qconfig defined, node observed, qconfig
...
def _compute_weight_qparams(
    ...
    if isinstance(float_module, torch.nn.RNNCellBase):
        ...
        wq_or_wq_dict.update(
            {
                "weight_ih": get_qparam_dict(weight_post_process_ih),
                "weight_hh": get_qparam_dict(weight_post_process_hh),
            }
        )
...
def _prepared_weight_post_process(
    float_module: torch.nn.Module,
    qconfig: QConfigAny,
    weight_post_process: Optional[torch.nn.Module],
    opts: _WeightConvertOptions,
    ...
    For PTQ (``weight_post_process is None``) a fresh observer is built from the
    qconfig and moved to the module's device. The observer is then run once on
    the weight unless we are in the PT2 QAT case where the shapes are already
    established.
    """
...
def _normalize_convert_configs(
    convert_custom_config: Union[ConvertCustomConfig, dict[str, Any], None],
    qconfig_mapping: Union[QConfigMapping, dict[str, Any], None],
    backend_config: Union[BackendConfig, dict[str, Any], None],
) -> tuple[ConvertCustomConfig, Optional[QConfigMapping], BackendConfig]:
    """Normalize the (possibly dict / None) config arguments to ``convert``.

    Emits the deprecation warnings for dict inputs and fills in defaults so the
    body of ``convert`` can assume typed config objects.
    """
...
def _convert_output_node(node: Node, ctx: _ConvertContext) -> None:
    ...
    if isinstance(output, (list, tuple)):
        for idx in output_quantized_idxs:
            _maybe_recursive_remove_dequantize(output[idx], return_node, graph)
        ...
        if 0 in output_quantized_idxs:
            _maybe_recursive_remove_dequantize(output, return_node, graph)
    else:
        warnings.warn(
...
def _reconcile_convert_qconfig_mapping(
    model: GraphModule,
    modules: dict[str, torch.nn.Module],
    ...
    return convert_node_name_to_qconfig
...
def convert(
    ...
    """
    convert_custom_config, qconfig_mapping, backend_config = _normalize_convert_configs(
        convert_custom_config, qconfig_mapping, backend_config
    )
    ...
    if qconfig_mapping:
        node_name_to_qconfig = _reconcile_convert_qconfig_mapping(
            model,
            modules,
            qconfig_mapping,
            observed_graph_module_attrs,
    ...
    # modules that convert_weighted_module is responsible for swapping
    weighted_module_classes = tuple(
        set(root_module_classes).union(qat_module_classes).union(fused_module_classes)
    )
