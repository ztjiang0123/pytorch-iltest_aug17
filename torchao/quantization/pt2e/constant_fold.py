...
class ConstantFolder(torch.fx.Interpreter):
    ...
    def _is_unfoldable_node(
        self, node: torch.fx.Node, flattened_inputs: list[Any]
    ) -> bool:
        """Return True when ``node`` must fold to ``unknown_value``.

        Collects the guard conditions that previously lived as a chain of
        early-returns in ``run_node`` so the main flow reads as a single check.
        """
        ...
        is_call_function = node.op == "call_function"
        # TODO - fix errors with this
        if is_call_function and node.target == aten._efficientzerotensor.default:
            return True

        # TODO - constant folding triton kernel returns the inputs -- fix this
        if is_call_function and node.name == "triton_kernel_wrapper_functional_proxy":
            return True
        ...
        return is_call_function and isinstance(
            node.target, torch._ops.HigherOrderOperator
        )

    def _record_replacement(self, node: torch.fx.Node, out: Any) -> None:
        """Register ``out`` as ``node``'s constant and update use bookkeeping."""
    ...
    def run_node(self, node: torch.fx.Node) -> Any:
        ...
        if self._is_unfoldable_node(node, flattened_inputs):
            return self.unknown_value
        ...
        is_foldable_const = not is_const_source(node, self.lifted_constant_names) and (
            isinstance(out, torch.Tensor) or out == self.deferred_value
        )
        if not is_foldable_const:
            return out
        ...
        self._record_replacement(node, out)
        return out
